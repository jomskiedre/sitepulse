...
class Event(db.Model):
    ...
    campaign=db.Column(db.String(200))
...
class GoogleConnection(db.Model):
    ...
    property_url=db.Column(db.String(1000),nullable=False)
    created_at=db.Column(db.DateTime,default=datetime.utcnow)
...
def site_for_user(site_id):
    u=require_user()
    s=Site.query.filter_by(id=site_id,owner_id=u.id).first()
    if not s: abort(404)
    return s

GSC_CLIENT_ID=os.environ.get("GOOGLE_CLIENT_ID")
GSC_CLIENT_SECRET=os.environ.get("GOOGLE_CLIENT_SECRET")
GSC_REDIRECT_URI=os.environ.get("GOOGLE_REDIRECT_URI")
GSC_SCOPE="https://www.googleapis.com/auth/webmasters.readonly"

def gsc_ready():
    return bool(GSC_CLIENT_ID and GSC_CLIENT_SECRET and GSC_REDIRECT_URI)

def refresh_gsc(conn):
    if conn.expires_at and conn.expires_at > datetime.utcnow()+timedelta(minutes=2):
    ...
    d=r.json(); conn.access_token=d["access_token"]
    conn.expires_at=datetime.utcnow()+timedelta(seconds=int(d.get("expires_in",3600)))
    db.session.commit()
    return conn.access_token
...
def gsc_query(conn, start_date, end_date):
    ...
    r=requests.post(url,headers={"Authorization":"Bearer "+token,"Content-Type":"application/json"},
        json={"startDate":start_date,"endDate":end_date,"dimensions":["query"],"rowLimit":25000},timeout=30)
    if not r.ok: return []
...
def signup():
    if request.method=="POST":
        ...
        if User.query.filter_by(email=email).first(): return render_template("signup.html",error="That email already has an account.")
...
def api_sites():
    ...
    if request.method=="GET":
        return jsonify(sites=[{"id":s.id,"name":s.name,"url":s.url} for s in Site.query.filter_by(owner_id=u.id).order_by(Site.id.desc()).all()])
    d=request.json or {}
    if not d.get("name") or not d.get("url"): return jsonify(error="Website name and URL are required"),400
    s=Site(owner_id=u.id,name=d["name"].strip(),url=d["url"].strip(),api_key=secrets.token_urlsafe(32)); db.session.add(s); db.session.commit()
    return jsonify(id=s.id,name=s.name,url=s.url,api_key=s.api_key)

@app.route("/api/sites/<int:site_id>/stats")
def stats(site_id):
    s=site_for_user(site_id); since=datetime.utcnow()-timedelta(days=int(request.args.get("days",30)))
    q=Event.query.filter(Event.site_id==s.id,Event.created_at>=since); views=q.filter_by(event_type="pageview").count(); clicks=q.filter_by(event_type="click").count()
    visitors=len({e.visitor_id for e in q.with_entities(Event.visitor_id).all() if e.visitor_id}); pages={}; elements={}; daily={}
    for e in q.all():
    ...
    keys=Keyword.query.filter_by(site_id=s.id).order_by(Keyword.clicks.desc()).limit(25).all()
    con=GoogleConnection.query.filter_by(user_id=current_user().id,site_id=s.id).first()
    return jsonify(site={"id":s.id,"name":s.name,"url":s.url},visitors=visitors,views=views,clicks=clicks,
        gsc_connected=bool(con),top_pages=sorted([{"name":k,"value":v} for k,v in pages.items()],key=lambda x:x["value"],reverse=True)[:10],
        top_clicks=sorted([{"name":k,"value":v} for k,v in elements.items()],key=lambda x:x["value"],reverse=True)[:10],
        daily=sorted([{"date":k,"value":v} for k,v in daily.items()]),
        keywords=[{"query":k.query,"clicks":k.clicks,"impressions":k.impressions,"ctr":k.ctr,"position":k.position} for k in keys])
...
def track():
    ...
    e=Event(site_id=s.id,event_type=d.get("event_type","pageview"),page=d.get("page"),element=d.get("element"),referrer=d.get("referrer"),source=d.get("source"),medium=d.get("medium"),campaign=d.get("campaign"),visitor_id=d.get("visitor_id")); db.session.add(e); db.session.commit()
    return jsonify(ok=True),200,{"Access-Control-Allow-Origin":"*"}

@app.route("/api/gsc/status")
def gsc_status():
    u=require_user(); con=GoogleConnection.query.filter_by(user_id=u.id).all()
    return jsonify(ready=gsc_ready(),connections=[{"site_id":c.site_id,"property_url":c.property_url} for c in con])
...
def gsc_connect(site_id):
    ...
    state=secrets.token_urlsafe(24); session["gsc_state"]=state; session["gsc_site_id"]=s.id
    params={"client_id":GSC_CLIENT_ID,"redirect_uri":GSC_REDIRECT_URI,"response_type":"code","scope":GSC_SCOPE,"access_type":"offline","prompt":"consent","state":state}
...
def gsc_callback():
    ...
    props=rr.json().get("siteEntry",[]); session["gsc_access_token"]=token; session["gsc_refresh_token"]=d.get("refresh_token"); session["gsc_props"]= [p.get("siteUrl") for p in props]
    return render_template("gsc_select.html",site=Site.query.get_or_404(site_id),properties=session["gsc_props"])
...
def gsc_save():
    ...
    if not con:
        con=GoogleConnection(user_id=u.id,site_id=site_id,property_url=prop,access_token=token,refresh_token=session.get("gsc_refresh_token"),expires_at=datetime.utcnow()+timedelta(hours=1)); db.session.add(con)
    else:
        con.property_url=prop; con.access_token=token; con.refresh_token=session.get("gsc_refresh_token") or con.refresh_token; con.expires_at=datetime.utcnow()+timedelta(hours=1)
    db.session.commit(); session.pop("gsc_access_token",None); session.pop("gsc_refresh_token",None); session.pop("gsc_props",None)
    return redirect("/dashboard")
...
def gsc_sync(site_id):
    ...
    for row in rows:
        db.session.add(Keyword(site_id=site_id,query=row.get("keys",[""])[0],clicks=int(row.get("clicks",0)),impressions=int(row.get("impressions",0)),ctr=float(row.get("ctr",0))*100,position=float(row.get("position",0))))
    db.session.commit(); return jsonify(synced=len(rows),period={"start":start.isoformat(),"end":end.isoformat()})

with app.app_context():
    db.create_all()
    admin_email=os.environ.get("ADMIN_EMAIL"); admin_pw=os.environ.get("ADMIN_PASSWORD")
    if admin_email and admin_pw and not User.query.filter_by(email=admin_email.lower()).first():
        db.session.add(User(email=admin_email.lower(),password_hash=generate_password_hash(admin_pw),role="admin")); db.session.commit()
